...
export default function BodyResetPage() {
  ...
  async function captureLead(event: React.FormEvent) {
    ...
    setStatus("saving");
    try { await submit("Lead Captured Before Quiz"); setLeadCaptured(true); setStatus("idle"); setTimeout(() => window.scrollTo({ top: 0, behavior: "smooth" }), 50); } catch { setStatus("error"); }
  }
  ...
  return <><main className="min-h-screen bg-cream pb-10 pt-20 md:pt-24">
    {!leadCaptured ? <section className="mx-auto max-w-6xl px-4 sm:px-6"><div className="grid overflow-hidden rounded-[24px] border border-purple/15 bg-white shadow-[0_12px_36px_rgba(60,40,80,0.09)] lg:grid-cols-[1.05fr_.95fr]">
      ...
      <div className="mt-4 flex justify-between gap-3"><button type="button" className="btn-secondary px-5 py-3" disabled={step === 1} onClick={() => setStep((value) => Math.max(1, value - 1))}>Back</button>{step < 5 ? <button type="button" className="btn-primary px-5 py-3" disabled={!canContinue} onClick={() => setStep((value) => Math.min(5, value + 1))}>Continue</button> : <button type="button" className="btn-primary px-5 py-3" disabled={!canContinue || status === "saving"} onClick={finish}>{status === "saving" ? "Thinking..." : "Show My Recommendation"}</button>}</div>
      {status === "error" && <p className="mt-3 text-center text-xs text-red-500">Something went wrong. Please try again or call Susie at (480) 440-0909.</p>}
    </div></section>}
