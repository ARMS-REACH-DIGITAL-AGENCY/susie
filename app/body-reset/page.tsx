...
import { treatmentOffers, type TreatmentKey, type TreatmentOffer, type TreatmentProduct } from "@/lib/treatment-offers";

type Key = TreatmentKey;
type Product = TreatmentProduct;
type Offer = TreatmentOffer;
...
const offers: Offer[] = treatmentOffers;
...
const emptyFields: Fields = { fullName: "", email: "", phone: "", serviceSmsConsent: false, marketingSmsConsent: false, symptoms: [], tried: [], goals: [], priority: "", urgency: "" };
...
export default function BodyResetPage() {
  ...
  async function submit(stage: string) {
    const response = await fetch("/api/submit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fullName: fields.fullName, email: fields.email, phone: fields.phone, consent: fields.serviceSmsConsent || fields.marketingSmsConsent, serviceSmsConsent: fields.serviceSmsConsent, marketingSmsConsent: fields.marketingSmsConsent, symptoms: fields.symptoms.join(", "), previouslyTried: fields.tried.join(", "), goals: fields.goals.join(", "), priority: fields.priority, urgency: fields.urgency, recommendedTreatment: result.offer.name, recommendationReasons: result.reasons.join(" | "), scoreSummary: result.summary, source: "website-evaluation", page: "/body-reset", stage }) });
    if (!response.ok) throw new Error("submit_failed");
  }

  async function captureLead(event: React.FormEvent) { event.preventDefault(); if (!fields.fullName || !fields.email) return; setStatus("saving"); try { await submit("Lead Captured Before Quiz"); setLeadCaptured(true); setStatus("idle"); setTimeout(() => window.scrollTo({ top: 0, behavior: "smooth" }), 50); } catch { setStatus("error"); } }
  ...
  return <><main className="body-reset-page min-h-screen bg-cream pb-10 pt-0">
    {!leadCaptured ? <section className="mx-auto max-w-6xl px-0 sm:px-6"><div className="relative min-h-[calc(100svh-5rem)] overflow-hidden bg-[#2c1f14] sm:rounded-[24px] sm:border sm:border-purple/15 sm:shadow-[0_12px_36px_rgba(60,40,80,0.12)]"><Image src="/images/symptom-yourself.png" alt="Woman looking thoughtfully in the mirror" fill priority className="object-cover object-center" /><div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(30,20,16,0.18)_0%,rgba(30,20,16,0.34)_38%,rgba(30,20,16,0.88)_100%)]" /><div className="relative z-10 flex min-h-[calc(100svh-5rem)] flex-col justify-end px-5 pb-5 pt-7 sm:px-7 sm:pb-7 lg:grid lg:grid-cols-[1.05fr_.95fr] lg:items-end lg:gap-10 lg:px-10 lg:pb-10"><div className="text-white"><p className="mb-3 inline-flex rounded-full border border-white/30 bg-black/20 px-3 py-1 font-sans text-[10px] font-semibold uppercase tracking-[0.18em] backdrop-blur-sm">See What Susie Says</p><h1 className="max-w-2xl font-serif text-[38px] font-light leading-[0.98] text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.55)] sm:text-5xl lg:text-6xl">Feel puffy, tired, foggy, inflamed, heavy, stuck, or unlike yourself?</h1><p className="mt-3 max-w-xl font-sans text-[14px] font-light leading-relaxed text-white/90 sm:text-base">Enter your information, answer five quick questions, and receive Susie’s professional starting-point recommendation.</p><p className="mt-2 font-sans text-sm font-medium text-white">Free. Private. No pressure.</p></div><form onSubmit={captureLead} className="mt-5 w-full rounded-[20px] border border-white/25 bg-[#2c1f14]/80 p-4 shadow-[0_16px_40px_rgba(0,0,0,0.28)] backdrop-blur-sm sm:p-5 lg:mt-0 lg:justify-self-end lg:max-w-[430px]"><div className="space-y-2.5"><input className="input-field bg-white/95" placeholder="Full Name*" value={fields.fullName} onChange={(e) => set("fullName", e.target.value)} required /><input className="input-field bg-white/95" type="email" placeholder="Email address*" value={fields.email} onChange={(e) => set("email", e.target.value)} required /><input className="input-field bg-white/95" type="tel" placeholder="Mobile number (optional)" value={fields.phone} onChange={(e) => set("phone", e.target.value)} /><button type="submit" className="btn-primary w-full py-3" disabled={status === "saving"}>{status === "saving" ? "Saving..." : "Receive My FREE Evaluation"}</button><details open className="rounded-md border border-white/20 bg-black/10 px-3 py-2"><summary className="cursor-pointer font-sans text-[11px] font-medium text-white/95">Optional SMS: RHONDA S BUTE (Susie Sculpts) — evaluation, appointments &amp; offers; message frequency varies; Msg &amp; data rates may apply; reply STOP to opt out or HELP for help.</summary><div className="mt-3 space-y-3"><label className="flex items-start gap-2 font-sans text-[10px] font-light leading-relaxed text-white/95 sm:text-[11px]"><input type="checkbox" className="mt-0.5 accent-purple" checked={fields.serviceSmsConsent} onChange={(e) => set("serviceSmsConsent", e.target.checked)} />I consent to receive non-marketing text messages from RHONDA S BUTE about my free evaluation results, purchase confirmations, appointments, scheduling, and customer service. Message frequency may vary, message &amp; data rates may apply. Text HELP for assistance, reply STOP to opt out.</label><label className="flex items-start gap-2 font-sans text-[10px] font-light leading-relaxed text-white/95 sm:text-[11px]"><input type="checkbox" className="mt-0.5 accent-purple" checked={fields.marketingSmsConsent} onChange={(e) => set("marketingSmsConsent", e.target.checked)} />I consent to receive marketing text messages from RHONDA S BUTE about follow-ups to my Body Reset Evaluation, treatment or package recommendations, purchase links, special offers, discounts, and service updates. Message frequency may vary. Message &amp; data rates may apply. Text HELP for assistance, reply STOP to opt out.</label></div></details><p className="text-center font-sans text-[10px] font-light leading-relaxed text-white/70 sm:text-[11px]">SMS consent is optional and is not a condition of purchase. View our <Link href="/privacy" className="underline underline-offset-2">Privacy Policy</Link> and <Link href="/terms" className="underline underline-offset-2">Terms &amp; Messaging Terms</Link>.</p>{status === "error" && <p className="text-center text-xs text-red-300">Something went wrong. Please try again or call Susie.</p>}</div></form></div></div></section> : <section className="mx-auto max-w-5xl px-3 sm:px-6"><div className="rounded-[20px] border border-purple/15 bg-white p-4 shadow-[0_10px_30px_rgba(60,40,80,0.08)] md:p-6"><div className="mb-4"><div className="mb-2 flex justify-between"><p className="section-label">Ask Susie Evaluation</p><p className="section-label">{step} of 5</p></div><div className="h-1.5 overflow-hidden rounded-full bg-stone/70"><div className="h-full bg-purple transition-all" style={{ width: `${step * 20}%` }} /></div></div>
      ...
      {step === 3 && <div><h2 className="font-serif text-2xl font-light text-[#2c1f14] md:text-3xl">What would feel like a real win?</h2><p className="mb-3 mt-1 text-xs font-light text-muted">Select everything that matters.</p><div className="grid gap-2 sm:grid-cols-2">{goals.map((option) => <ChoiceButton key={option} label={option} selected={fields.goals.includes(option)} onClick={() => toggle("goals", option)} />)}</div></div>}
