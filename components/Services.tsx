...
function TreatmentIcon({ type, compact = false }: { type: string; compact?: boolean }) {
  ...
  const size = compact ? "h-12 w-12 sm:h-14 sm:w-14" : "h-56 w-56 lg:h-64 lg:w-64 xl:h-72 xl:w-72";
...
function TreatmentDetails({ treatment }: { treatment: (typeof treatments)[number] }) {
  return (
    <div className="flex h-full flex-col">
      ...
      <Link href="/body-reset" className="btn-primary mt-auto w-full text-center" onClick={(event) => event.stopPropagation()}>
  ...
  return (
    <section className="bg-cream pb-12 pt-10 md:pb-16 md:pt-14">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div id="treatments" className="mb-4 scroll-mt-20 text-center md:mb-9 md:scroll-mt-24">
          <p className="section-label mb-3">The Ultimate &quot;YOU&quot; Experience</p>
          <h2 className="whitespace-nowrap font-serif text-[18px] font-light leading-tight text-[#2c1f14] sm:text-3xl md:text-5xl">Explore Susie&apos;s Professional Treatments</h2>
        </div>

        <div className="relative md:hidden">
          <div className="sticky top-16 z-30 -mx-4">
            <div className="border-y border-purple/10 bg-cream/95 px-2 py-2 shadow-sm backdrop-blur-md">
              <div className="grid grid-cols-6 gap-1">
                {treatments.map((treatment, index) => (
                  <button key={treatment.title} type="button" onClick={() => setSelected(index)} className={`flex min-h-[96px] flex-col items-center justify-center rounded-[10px] border px-0.5 py-2 text-center transition ${selected === index ? "border-purple bg-purple/10 shadow-sm" : "border-stone/80 bg-white/75 text-muted"}`} aria-pressed={selected === index} aria-label={treatment.title}>
                    <TreatmentIcon type={treatment.icon} compact />
                    <span className={`mt-1 flex min-h-[16px] w-full items-center justify-center whitespace-nowrap font-sans text-[7px] font-medium uppercase leading-none tracking-[0.01em] sm:text-[8px] ${selected === index ? "text-purple" : "text-muted"}`}>{treatment.tag}</span>
                  </button>
                ))}
              </div>
            </div>
            <div className="mx-4 rounded-b-[22px] border border-t-0 border-purple/15 bg-white/95 p-5 shadow-[0_10px_28px_rgba(60,40,80,0.08)] backdrop-blur-md">
          ...
          {treatments.map((treatment, index) => {
            ...
            return (
              <div
                ...
                aria-label={`${isFlipped ? "Hide" : "Show"} details for ${treatment.title}`}
                ...
                className="h-[540px] cursor-pointer rounded-[22px] outline-none transition duration-300 hover:-translate-y-2 hover:shadow-[0_22px_45px_rgba(60,40,80,0.18)] focus-visible:ring-2 focus-visible:ring-purple focus-visible:ring-offset-4 [perspective:1200px]"
              >
                <div className={`relative h-full w-full transition-transform duration-700 [transform-style:preserve-3d] ${isFlipped ? "[transform:rotateY(180deg)]" : ""}`}>
                  <div className="absolute inset-0 flex flex-col items-center justify-center rounded-[22px] border border-purple/15 bg-gradient-to-br from-white via-cream to-purple/5 p-7 text-center shadow-[0_10px_28px_rgba(60,40,80,0.08)] [backface-visibility:hidden]">
                    <div className="mb-1"><TreatmentIcon type={treatment.icon} /></div>
                    <p className="section-label mb-3">{treatment.tag}</p>
                    <h3 className="font-serif text-3xl font-light leading-tight text-[#2c1f14]">{treatment.title}</h3>
                    <p className="mt-5 font-sans text-xs font-medium uppercase tracking-[0.12em] text-purple/70">Tap to learn more</p>
                  </div>
        ...
        <p className="mx-auto mt-8 max-w-xl text-center font-sans text-xs font-light text-muted/60">Every treatment supports a different goal, are wellness-focused, and are not intended to diagnose, treat, cure, or prevent any disease. Individual experiences vary.</p>
      </div>
    </section>
  );
}
