...
function TreatmentIcon({ type, compact = false }: { type: string; compact?: boolean }) {
  ...
  const index = iconIndex[type] ?? 0;
  const size = compact
    ? "h-11 w-11 sm:h-12 sm:w-12"
    : "h-40 w-40 lg:h-44 lg:w-44 xl:h-48 xl:w-48";

  return (
    <span className={`relative block shrink-0 overflow-hidden rounded-full ${size}`} aria-hidden="true">
      <img
        src="/images/treatment-icons-sprite.webp"
        alt=""
